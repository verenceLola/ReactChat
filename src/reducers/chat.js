import { FETCHING_CHAT, FETCHING_CHAT_FAILED, FETCHING_USER_CHAT_SUCCESS, NEW_MESSAGE, UPDATE_ACTIVE_CHAT } from "../actions/Chat/types";
...
export default (state=INITIAL_STATE, action) => {
    switch (action.type) {
        ...
        case FETCHING_USER_CHAT_SUCCESS:
            return {...state, chats: [...state.chats, ...action.chats], activeChat: action.chats[0]}
        case NEW_MESSAGE:
            const { chatId } = action.message
            from(state.chats).find(chat => chat.id === chatId)
